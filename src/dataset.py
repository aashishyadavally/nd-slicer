'''Contains ``torch.utils.data.Dataset`` objects for all experiments.
'''
import json
import pickle
import random
...
from utils import (
    code_to_method_calls, code_to_method_definitions, code_to_ranges, code_to_returns,
    extract_all_variables, extract_dynamic_slice_with_reduced_trace, extract_import_lines,
    extract_reaching_statements, extract_trace, extract_variable_flow_from_pdg,
    get_ast_parser, string_to_tokens
)
...
class CrashDetectionInputFeatures:
    '''A single training/test features for a example for extrinsic evaluation.
    '''

    def __init__(
        self, id, code, code_tokens, slice_tokens, zero_var, criterion, occurrence,
        reaching_statements,
    ):
        self.id = id
        self.code = code
...
class InputFeatures:
    ...
    def __init__(self, id, code, code_tokens, trace_tokens, slice_tokens, criterion, occurrence):
        self.id = id
        self.code = code
        self.code_tokens = code_tokens
...
class InterMethodInputFeatures:
    '''A single training/test features for a example for inter-method analysis.
    '''

    def __init__(self, id, code_tokens, trace, criterion, occurrence, call_linkage):
        self.id = id
        self.code_tokens = code_tokens
        self.trace = trace
        self.criterion = criterion
...
class PointerNetworkInputFeatures:
    '''A single training/test features for a example in a Pointer Network Model.
    '''

    def __init__(self, id, code, code_tokens, pointer_labels, slice_tokens, criterion, occurrence):
        self.id = id
        self.code = code
        self.code_tokens = code_tokens
...
class QEInputFeatures:
    '''A single training/test features for a example for qualitative evaluation.
    '''

    def __init__(self, id, code, code_tokens, slice_tokens, criterion, occurrence, range):
        self.id = id
        self.code = code
        self.code_tokens = code_tokens
        self.slice_tokens = slice_tokens
...
def convert_examples_to_features(js):
    '''Create input features for intrinsic evaluation on executable code.
    
    Arguments:
        js (dict): Data instance.

    Returns:
        (InputFeatures): Input features for intrinsic evaluation on executable code. 
    '''
...
def convert_examples_to_partial_features(js):
    '''Create input features for intrinsic evaluation on non-executable code.
    
    Arguments:
        js (dict): Data instance.

    Returns:
        (PartialInputFeatures): Input features for intrinsic evaluation on non-executable code. 
    '''
...
def convert_examples_to_pointer_features(js):
    '''Create input features for intrinsic evaluation on executable code using Pointer Networks.
    
    Arguments:
        js (dict): Data instance.

    Returns:
        (PointerNetworkInputFeatures): Input features for intrinsic evaluation on non-executable
            code using Pointer Networks.
    '''
...
def convert_zero_division_examples_to_features(js, tokenizer):
    '''Create input features for extrinsic evaluation on crash detection.
    ...
    Returns:
        (CrashDetectionInputFeatures): Input features for intrinsic evaluation on non-executable
            code using Pointer Networks.
    '''
    stopping_line = js['not_zero_line']['line']
    numerator_vars = [var for var in js['not_zero_line']
                      ['vars'] if var != js['zero_var']]
    ...
    indent_dedents_string = ' ' * \
        (len(raw_code_lines[criterion]) -
         len(raw_code_lines[criterion].lstrip()))
    ...
    reaching_statements = extract_reaching_statements(
        js['zero_var'], criterion, injected_code)

    variable_flow = extract_variable_flow_from_pdg(injected_code)
    try:
        backward_slice = extract_dynamic_slice_with_reduced_trace(
            criterion, crash_trace, variable_flow
        )
    except:
        return None
...
class CodeNetTextDataset(Dataset):
    '''``torch.utils.data.Dataset`` item for intrinsic evaluation on executable code.
    '''
    def __init__(self, tokenizer, args, mode, logger):
        ...
        if args.do_eval_base:
            cached_features_file = Path(
                args.data_dir) / f"feats_base_{mode}.pkl"
        else:
            if args.encoder == 'graphcodebert' and args.decoder == 'graphcodebert':
                cached_features_file = Path(
                    args.data_dir) / f"feats_gcb_{mode}.pkl"
            else:
                cached_features_file = Path(
                    args.data_dir) / f"feats_{mode}.pkl"
        ...
        if Path(cached_features_file).is_file():
            ...
            logger.info(
                f"Load and create features from dataset file at {filename}")
            ...
            with open(str(filename), "r", encoding="utf-8") as f:
                for line in tqdm(f, total=num_lines):
                    ...
                    if len(json_line['code_tokens']) != 0:
                        trace_tokens = [
                            f"<{trace_line['line']}>" for trace_line in json_line['trace']]
                            ...
                            for occurrence, slice in enumerate(criterion_slices):
                                if len(slice) != 1:
                                    ...
                                    code_tokens = tokenizer.tokenize(
                                        " ".join(code_tokens))
                                    slice_tokens = [
                                        f"<{slice_line}>" for slice_line in slice]
                                    js = {
                                    ...
                                    try:
                                        features = convert_examples_to_features(js)
            ...
            with open(str(Path(args.data_dir) / cached_features_file), 'wb') as handle1:
                pickle.dump(self.examples, handle1,
    ...
    def __getitem__(self, item):
        ...
        source_tokens = js.code_tokens[: max_source_size -
                                       9 - len(occurrence_tokens)]
        source_tokens = ["<s>", "<encoder-decoder>", "</s>"] + source_tokens + ["</s>"] + \
            criterion_token + ["</s>"] + \
            occurrence_tokens + ["<mask0>", "</s>"]
        source_ids = self.tokenizer.convert_tokens_to_ids(source_tokens)
        source_padding_length = max_source_size - len(source_ids)
        source_ids += [self.tokenizer.pad_token_id for _ in range(
            source_padding_length)]
        ...
        gold_ids += [self.tokenizer.pad_token_id for _ in range(
...
class CrashDetectionDataset(Dataset):
    '''``torch.utils.data.Dataset`` item for extrinsic evaluation on crash detection.
    '''
    def __init__(self, tokenizer, args, mode, logger):
        self.args = args
        self.tokenizer = tokenizer
        ...
        if Path(cached_features_file).is_file():
            ...
            with open(str(filename), "r", encoding="utf-8") as f:
                for line in tqdm(f, total=num_lines):
                    ...
                    if where_zero_found == not_zero_line['line']:
                        continue
                    ...
                    try:
                        features = convert_zero_division_examples_to_features(
                            js, tokenizer)
...
class InterMethodDataset(Dataset):
    '''``torch.utils.data.Dataset`` item for qualitative evaluation on inter-method analysis.
    '''
    def __init__(self, tokenizer, args, mode, logger):
        self.args = args
        self.tokenizer = tokenizer
        self.ast_parser = get_ast_parser('../data/build')

        cached_features_file = Path(args.data_dir) / f"feats_{mode}_im.pkl"
        filename = Path('../data/codenetmut_test.json')
        ...
        if Path(cached_features_file).is_file():
            ...
            with open(str(filename), "r", encoding="utf-8") as f:
                for line in tqdm(f, total=num_lines):
                    if line in dataset_ids:
                        continue
                    try:
                        json_line = json.loads(line)
                        tree = self.ast_parser.parse(
                            bytes(json_line['code'], 'utf8'))
                        code_lines = json_line['code'].split('\n')
                        method_definitions = code_to_method_definitions(
                            tree.root_node)
                        method_definitions = [{
                            'identifier': code_lines[defn['start_line']][defn['start_idx']: defn['end_idx']],
                        ...
                        identifiers = extract_all_variables(
                            self.ast_parser, json_line['code'])
                        ...
                        method_calls = [item for item in method_calls if item['identifier'] in
                                        [defn['identifier'] for defn in method_definitions]]
                        return_statements = [x['line']
                                             for x in code_to_returns(tree.root_node)]
                                ...
                                if defn['start_line'] <= call['line'] <= defn['end_line']:
                                    for _defn in method_definitions:
                                        if _defn['identifier'] == call['identifier']:
                                            callee_start_line, callee_end_line = _defn[
                                                'start_line'], _defn['end_line']
                        ...
                        code_tokens = tokenizer.tokenize(
                            " ".join(json_line['code_tokens']))
                        trace = [x['line']
                                 for x in extract_trace(json_line['trace'])]
                        ...
                        for linkage in code_linkages:
                            ...
                            if call['line'] not in lines_to_variable:
                                continue
                            ...
                            for criterion in list(set(criterion_lines)):
                                if criterion not in trace or linkage['call_line'] not in trace:
                                    continue
                                if criterion < linkage['calling_start_line'] or criterion > linkage['calling_end_line']:
                                    continue
...
class PartialCodeNetTextDataset(CodeNetTextDataset):
    '''``torch.utils.data.Dataset`` item for intrinsic evaluation on non-executable code.
    '''
    def __init__(self, tokenizer, args, mode, logger):
        self.args = args
        self.tokenizer = tokenizer
        self.ast_parser = get_ast_parser('../data/build')
        cached_features_file = Path(
            args.data_dir) / f"feats_{mode}_partial.pkl"
        ...
        if Path(cached_features_file).is_file():
            ...
            with open(str(filename), "r", encoding="utf-8") as f:
                for line in tqdm(f, total=num_lines):
                    json_line = json.loads(line)
                    import_lines = sorted(extract_import_lines(
                        self.ast_parser, json_line['code']))
                    if not import_lines:
                        continue
                    ...
                    importless_code_tokens = string_to_tokens(
                        importless_code, tokenizer)
                    ...
                    for criterion, criterion_slices in json_line['slices'].items():
                        if int(criterion) not in line_offset_mapper:
                            continue

                        for occurrence, slice in enumerate(criterion_slices):
                            importless_slice_tokens = [f"<{line_offset_mapper[slice_line]}>"
                                                       for slice_line in slice if slice_line in line_offset_mapper]
                            if len(importless_slice_tokens) <= 1:
                                continue

                            importless_criterion = str(
                                line_offset_mapper[int(criterion)])
                            js = {
                            ...
                            try:
                                features = convert_examples_to_partial_features(js)
                                total_num += 1
                                self.examples.append(features)
                            except:
                                error_num += 1
                ...
                for line in tqdm(f, total=num_lines):
                    ...
                    for criterion, criterion_slices in json_line["slices"].items():
                        for occurrence, slice in enumerate(criterion_slices):
                            ...
                            try:
                                features = convert_examples_to_pointer_features(js)
                                self.examples.append(features)
                            except:
                                error_num += 1
...
class QETextDataset(CodeNetTextDataset):
    '''``torch.utils.data.Dataset`` item for qualitative evaluation.
    '''
    def __init__(self, tokenizer, args, mode, logger, node_type, partial=False):
        ...
        if not partial:
            ...
            cached_qe_file = Path(args.data_dir) / \
                f"feats_qe_{node_type}_{mode}.pkl"
        else:
            cached_features_file = Path(
                args.data_dir) / f"feats_{mode}_partial.pkl"
            cached_qe_file = Path(args.data_dir) / \
                f"feats_qe_{node_type}_{mode}_partial.pkl"

        if Path(cached_qe_file).is_file():
            logger.warning(
                f"Loading features from cached file for qualitative evaluation {cached_qe_file}")
            ...
            for example in input_examples:
                if not partial:
                    node_ranges = code_to_ranges(
                        tokenizer, [node_type], code_tokens=example.code_tokens)[node_type]
                else:
                    node_ranges = code_to_ranges(
                        tokenizer, [node_type], code=example.code)[node_type]

                for nrange in node_ranges:
                    if nrange[0] - 1 < example.criterion < nrange[1] - 1:
                        self.examples.append(QEInputFeatures(
                            id=example.id,
                            code=example.code,
            ...
            logger.warning(
                f"Num examples for {node_type} = {len(self.examples)}")
            logger.warning(
                f"Saving features into cached file {cached_qe_file}")
